...
// Factory keys. Used to initialize the enclave with a set of keys. First key is the master key. Rest are encrypted keys.
static uint8_t enclave_factory_keys[ENCLAVE_FACTORY_KEY_SLOTS + 1][32] = {
...
static const uint8_t enclave_signature_expected[ENCLAVE_FACTORY_KEY_SLOTS][ENCLAVE_SIGNATURE_SIZE] = {
    {0x20, 0x40, 0x43, 0x7c, 0x0a, 0xd3, 0x4b, 0x76, 0x86, 0x7f, 0x00, 0x83, 0x0e, 0x18, 0xe2, 0xbc}, 
    {0x58, 0x1c, 0xc3, 0xb4, 0xf2, 0xe7, 0x57, 0xbe, 0x33, 0x61, 0x59, 0xf9, 0xfd, 0x30, 0x01, 0xd2}, 
    {0x04, 0x5a, 0xa9, 0xde, 0x95, 0x41, 0x8d, 0x8e, 0xa8, 0xeb, 0x5d, 0x2e, 0xfe, 0xd5, 0x12, 0x2d}, 
    {0x8b, 0x2d, 0x4a, 0xb8, 0x3c, 0x8d, 0x64, 0xb9, 0x37, 0x15, 0x60, 0x26, 0x35, 0x09, 0xe5, 0x13}, 
    {0x7a, 0x7b, 0x42, 0x98, 0x27, 0xc8, 0xfd, 0xb4, 0xbc, 0x2f, 0x50, 0x2e, 0x8a, 0x1e, 0x87, 0xd1}, 
    {0x89, 0x20, 0x61, 0x0f, 0xba, 0x42, 0x4f, 0x7a, 0x5b, 0x3e, 0x65, 0xee, 0x74, 0x10, 0x8c, 0x7c}, 
    {0x8e, 0x1f, 0x1c, 0x7d, 0x42, 0xcc, 0xec, 0x84, 0xe7, 0x53, 0x42, 0x9c, 0x01, 0xa6, 0xab, 0x78}, 
    {0x24, 0xc5, 0x65, 0x1f, 0x07, 0xba, 0xd3, 0x2c, 0x50, 0xa1, 0x4f, 0xfd, 0xc9, 0xc2, 0x21, 0x75}, 
    {0x6d, 0x27, 0x29, 0xf6, 0xf8, 0x60, 0x4a, 0xc7, 0xcd, 0x96, 0xed, 0xcf, 0x67, 0x03, 0xa8, 0xf5}, 
    {0xe8, 0xaa, 0xac, 0xca, 0xc2, 0x25, 0xdf, 0x4d, 0xe8, 0x29, 0x74, 0xc2, 0x6e, 0x1e, 0xfc, 0xcc},
};
...
static bool furi_hal_crypto_generate_factory_keys(uint8_t start_slot, uint8_t end_slot) {
    FuriHalCryptoKey key;
    FURI_LOG_I(TAG, "Generating factory keys %u..%u", start_slot, end_slot);
...
bool furi_hal_crypto_enclave_verify(uint8_t* keys_nb, uint8_t* valid_keys_nb) {
    ...
    // 验证出厂密钥前，先看看是否有出厂密钥，没有就载入
    if(furi_hal_crypto_enclave_load_key(1, enclave_signature_iv[1])) {
        furi_hal_crypto_enclave_unload_key(1);
    } else {
        furi_hal_crypto_generate_factory_keys(0, ENCLAVE_FACTORY_KEY_SLOTS);
    }

    // 验证出厂密钥，使用0-10号密钥与代码头部的IV对头部的签名进行AES256CBC加密，然后与预期的签名进行比较
